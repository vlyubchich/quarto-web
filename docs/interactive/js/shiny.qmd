...
In the example above we took advantage of the fact that by default OJS `viewof` expressions are propagated to Shiny inputs (e.g `input$xcol`). This provides a reasonable separation of concerns, and prevents excess network traffic in the case that you have large OJS reactives.

However, if you want to use other OJS reactives as Shiny inputs this is also possible using the `ojs-exports` option. The default behavior maps to the following configuration:
...
You can also specify `ojs-exports: all` to cause all OJS reactives to be bound to Shiny inputs:
...
Alternatively, you can specify a list of OJS reactives by name (including using `!` to filter out reactives), and combine this with the `viewof` and `all` options. For example:

``` yaml
---
server:
  type: shiny
  ojs-exports: [all, !large_dataset]
---
```
