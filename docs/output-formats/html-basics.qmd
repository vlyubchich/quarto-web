...
If you want to produce a fragment of HTML suitable for inclusion in another document rather than an entire HTML document, you can use the `standalone: false` option. For example:
...
It's important to note though that many features will be disabled for this mode, as many of the more advanced formatting and interactivity behaviors of `html` require injecting CSS and JavaScript into the document `<head>` tag (and `standalone: false` produces only a fragment rather than a full document with `<head>` and `<body>`). Features not available for `standalone: false` include:

-   All CSS styling including the use of Bootstrap themes
-   Quarto figure and panel layout (e.g. `layout-ncol: 2`)
-   Anchor sections, reference popups, tabsets, code block copying, and responsive figures.

Another approach to creating fragments of HTML for inclusion in other documents is to use the [Minimal HTML](#minimal-html) feature described below, copying the CSS and JS files included in the `<head>` into the parent document where the HTML will be embedded. This approach is a bit more effort but has the benefit of preserving the features described above (save for Bootstrap themes).
